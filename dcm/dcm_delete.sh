...
dcm_delete_test1()
{
	mosquitto_pub -m '{"action":"delete"}' -t "glp/0/${SID}/rq/con/test30/do"
}
...
dcm_delete_test2()
{
	...
	pids+=($!)
	mosquitto_pub -m '{"action":"delete"}' -t "glp/0/${SID}/rq/con/test20/do"
	parse_sts "${DELETED_STATE}"
	result "sts"
	pub=("mosquitto_pub -m '{"action":"delete"}' -t '"glp/0/${SID}/rq/con/test20/do"'")
...
dcm_delete_test3()
{
	...
	pids+=($!)
	mosquitto_pub -m '{"action":"delete"}' -t "glp/0/${SID}/rq/con/test11/do"
	parse_sts "${DELETED_STATE}"
	result "sts"
	pub=("mosquitto_pub -m '{"action":"delete"}' -t '"glp/0/${SID}/rq/con/test11/do"'")
	result_logs "dcm_delete_test" ${LOG_DCM_DELETE}  ${start_time} "${pub}"
}
