...
dio_create_test2()
{
	...
	subscribe_logger_event &
	pids+=($!)
	mosquitto_pub -m '{"action":"create","args":{"type":"ABCDEFG"}}' -t "glp/0/${SID}/rq/dev/${IOX_PROTOCOL}/${DIO_DEV_NAME}/do"
	parse_logger
	result "logger"
	pub=("mosquitto_pub -m '{"action":"create","args":{"type":"ABCDEFG"}}' -t '"glp/0/${SID}/rq/dev/${IOX_PROTOCOL}/${DIO_DEV_NAME}/do"'")
	result_logs "dio_create_test" ${LOG_DIO_CREATE}  ${start_time} "${pub}"
}
...
dio_create_test3()
{
	...
	parse_logger
	result "logger"
	pub=("mosquitto_pub -m '{"action":"create","args":{"type":"dio"}}' -t '"glp/0/${SID}/rq/dev/${IOX_PROTOCOL}/${DIO_DEV_NAME}/do"'")
	result_logs "dio_create_test" ${LOG_DIO_CREATE}  ${start_time} "${pub}"
}

#Create on same handle with same type
...
#Create on same handle with diff type
...
#Create with provision flag false but with unid
...
#Create with provision true and correct unid
