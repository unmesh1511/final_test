...
modbus_message_test1()
{
	...
	pids+=($!)
	mosquitto_pub -m '{"action":"message"}' -t "glp/0/${SID}/rq/dev/${MODBUS_PROTOCOL}/pm820/do"
	parse_logger
	result "logger"
	pub=("mosquitto_pub -m '{"action":"message"}' -t '"glp/0/${SID}/rq/dev/${MODBUS_PROTOCOL}/pm820/do"'")
