...
modbus_repair_test1()
{
	...
	pids+=($!)
	mosquitto_pub -m '{"action":"repair"}' -t "glp/0/${SID}/rq/dev/${MODBUS_PROTOCOL}/pm820/do"
	parse_logger "${REPAIR_NOT_FOUND}"
	result "logger"
	pub=("mosquitto_pub -m '{"action":"repair"}' -t '"glp/0/${SID}/rq/dev/${MODBUS_PROTOCOL}/pm820/do"'")
